...
class JobController extends Controller
{
    public function jobList($nav_category = null){
        ...
        $categories = Navigation::where('page_type','Group Jobs')->where('page_status',1)->get();
        //return $navigations;
    ...
    public function store(Request $request, $nav_category)
    {
        ...
        $foodaccommondation = $request ['foodaccommondation'];
        $total_demand = $request ['total_demand'];

        $working_day = $request['working_day'];
        $working_hour = $request ['working_hour'];
        $visa_ticket = $request['visa_ticket'];
        $service_charge = $request ['service_charge'];
        $lt_number = $request ['lt_number'];
        $interview = $request ['interview'];
          ...
          unset($request['posted_on']);
          unset($request['foodaccommondation']);
          unset($request['total_demand']);
          unset($request['working_day']);
          unset($request['working_hour']);
          unset($request['visa_ticket']);
          unset($request['service_charge']);
          unset($request['lt_number']);
          unset($request['interview']);
     ...
     public function JobUpdate(Request $request, $nav_category, $id)
    {
        ...
        //return $navigations;
        return redirect('admin/navigation-list/'.$nav_category.$parent_id)->with('success','Data Updated Succssfully!!');
